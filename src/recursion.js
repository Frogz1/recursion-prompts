...
var factorial = function(n) {
    //inputs :n (n-1) number to factorial (5 * 4 * 3 * 2...)
...
var arraySum = function(array) {
    //input , nested arrays, need to get values out of arrays
    //base case: if item does not equal array add it
    var sum = 0;
...
var createArray = function(str) {
    var result ;
    var recurse = function(chars, arr) {
        if (chars === str.length) {
            result = arr;
            return;
        }
        var val = [str[chars]];
        return recurse(chars + 1, arr.concat(val));
    };
     recurse(0, []);
     return result;
};
